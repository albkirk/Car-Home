...
unsigned long TimeOUT = 35000;                  // [ms] time to consider not having the BT beacon present 
BLEScan* pBLEScan;
...
void BLEScan_loop(){
    telnet_println("fn_task_BLEScan running on core " + String(xPortGetCoreID()));
    ...
    telnet_println("Scan done  --  Last Scan time: " + String(millis()/1000));
}

void fn_task_BLEScan(void *parameters){
    telnet_println("fn_task_BLEScan running on core " + String(xPortGetCoreID()));
    delay(uint32_t(scanTime * 1000 * 2));
    BLEDevice::init("");
...
void project_setup() {
  // Start Ambient devices
    buttons_setup();
    xTaskCreate(fn_task_BLEScan, "task_BLEScan", 4096, (void *)NULL, 0, &task_BLEScan);
    //if(eTaskGetState(task_BLEScan) ==4) {telnet_println("Valor 4"); vTaskDelete(task_BLEScan);}
    //if(eTaskGetState(task_BLEScan) ==1)  telnet_println(" Task Status: " + String(eTaskGetState(task_BLEScan)) );
